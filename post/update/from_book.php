...
if(isset($data['unico_id']) && $HTTP_REFERER){
    ...
    $check_post_sql = "SELECT id,image_path FROM {$category_table} WHERE id = {$post_id} AND user_id = {$user_id} ";
    $check_post = $conn->query($check_post_sql);
    if($check_post->rowCount()){
    	$db_image = $check_post->fetch();
    	$db_image = $db_image["image_path"];
        if(em($post_text) && em($data["image"]) && em($db_image)){
		    ...
		    if(!em($data["image"])){
	            ...
	            // put base64 image
	            $putImage = new putImage64($base64String,"../../media/images/category/from-book/");
	            $image_path = "from-book/".$putImage->getPath();
	        ...
	        $update_post_sql = "UPDATE {$category_table} 
			...
			if($update_post){
                ...
                $url = "$api_host/website/archives/post/from_book.php?id=$post_id";
                $ch = curl_init();
