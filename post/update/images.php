...
if(isset($data['unico_id']) && $HTTP_REFERER){
    ...
    $post_title = str_replace(array("\r\n","\r","\n","\\r","\\n","\\r\\n"),"<br/>",$post_title);
    $post_title = local_filter_post($post_title);
    ...
    if($check_post->rowCount()){
        if(!em($data["image"])){
            ...
            // put base64 image
            $putImage = new putImage64($base64String,"../../media/images/category/images/");
            $image_path = $putImage->getPath();
		...
		if($update_post){
			echo json_encode(["mess" => 'تم التعديل',"post_title" => $post_title]);
            // update post in archives
            $url = "$api_host/website/archives/post/images.php?id=$post_id";
            $ch = curl_init();
